...
export class GenreQueryService {
  ...
  private getGenreHistoryByAccountQuery: GetGenreHistoryByAccountQuery
  ...
  constructor(db: IDrizzleConnection) {
    ...
    this.getGenreHistoryByAccountQuery = new GetGenreHistoryByAccountQuery(db)
  ...
  getGenreHistoryByAccount(accountId: number) {
    return this.getGenreHistoryByAccountQuery.execute(accountId)
  }
