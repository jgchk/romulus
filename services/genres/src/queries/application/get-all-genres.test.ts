...
test('should return an array of genres when there are genres in the DB', async ({
  ...
  await createGenre(getTestGenre(), 1, dbConnection)
...
test('should paginate the results', async ({ dbConnection }) => {
  await Promise.all(
    ...
    ].map((genre) => createGenre(genre, 1, dbConnection)),
  )
...
test('should handle a limit that is larger than the number of genres in the DB', async ({
  ...
  await createGenre(getTestGenre({ name: 'Test' }), 1, dbConnection)
...
test('should include parent ids when requested', async ({ dbConnection }) => {
  ...
  const test1 = await createGenre(getTestGenre({ name: 'Test 1' }), accountId, dbConnection)
  await createGenre(
...
test('should include akas when requested', async ({ dbConnection }) => {
  ...
  await createGenre(getTestGenre({ name: 'Test 2' }), accountId, dbConnection)
...
test('should allow filtering on name by exact match', async ({ dbConnection }) => {
  ...
  await createGenre(getTestGenre({ name: 'Test 1' }), accountId, dbConnection)
  await createGenre(getTestGenre({ name: 'Test 2' }), accountId, dbConnection)
...
test('should allow filtering by createdBy', async ({ dbConnection }) => {
  ...
  await createGenre(getTestGenre({ name: 'Test 1' }), account1Id, dbConnection)
  await createGenre(getTestGenre({ name: 'Test 2' }), account2Id, dbConnection)

  const query = new GetAllGenresQuery(dbConnection)
  const result = await query.execute({ filter: { createdBy: account1Id } })
...
test('should allow filtering by both createdBy and name', async ({ dbConnection }) => {
  ...
  await Promise.all(
    [getTestGenre({ name: 'Test 1' }), getTestGenre({ name: 'Test 2' })].map((genre) =>
      createGenre(genre, accountId, dbConnection),
    ),
  )

  const query = new GetAllGenresQuery(dbConnection)
  const result = await query.execute({ filter: { name: 'Test 1', createdBy: accountId } })
...
test('should allow filtering by multiple parent ids', async ({ dbConnection }) => {
  ...
  const parent1 = await createGenre(getTestGenre({ name: 'Parent 1' }), accountId, dbConnection)
  const parent2 = await createGenre(getTestGenre({ name: 'Parent 2' }), accountId, dbConnection)
  await createGenre(getTestGenre({ name: 'Child 1' }), accountId, dbConnection)
  await createGenre(
...
test('should allow filtering by parents and createdBy', async ({ dbConnection }) => {
  ...
  const parent1 = await createGenre(getTestGenre({ name: 'Parent 1' }), account1Id, dbConnection)
  const parent2 = await createGenre(getTestGenre({ name: 'Parent 2' }), account1Id, dbConnection)
  await createGenre(getTestGenre({ name: 'Child 1' }), account1Id, dbConnection)
  await createGenre(
    getTestGenre({ name: 'Child 2', parents: new Set([parent1.id]) }),
    account1Id,
    dbConnection,
  ...
  const query = new GetAllGenresQuery(dbConnection)
  const result = await query.execute({
    filter: { parents: [parent1.id, parent2.id], createdBy: account2Id },
  })
...
test('should allow filtering by ancestor id', async ({ dbConnection }) => {
  ...
  const other1 = await createGenre(getTestGenre({ name: 'Other 1' }), accountId, dbConnection)
  await createGenre(
...
test('should allow filtering by multiple ancestor ids', async ({ dbConnection }) => {
  ...
  await createGenre(getTestGenre({ name: 'Other 1' }), accountId, dbConnection)
  await createGenre(
...
test('should allow sorting by id', async ({ dbConnection }) => {
  await Promise.all(
    [getTestGenre({ name: 'A' }), getTestGenre({ name: 'B' }), getTestGenre({ name: 'C' })].map(
      (genre) => createGenre(genre, 1, dbConnection),
    ),
  )
