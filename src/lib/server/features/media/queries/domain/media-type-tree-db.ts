    ...
    if (event.baseTreeId !== undefined) {
      ...
      if (mediaTypes.length > 0) {
        await this.db
          ...
          .values(mediaTypes.map((mediaType) => ({ ...mediaType, mediaTypeTreeId: event.treeId })))
      }
      ...
      if (relations.length > 0) {
        await this.db
          ...
          .values(relations.map((relation) => ({ ...relation, mediaTypeTreeId: event.treeId })))
      }
    }
  }
  ...
  private async removeMediaType(event: MediaTypeRemovedEvent): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      ...
      const relationships = parents.flatMap((parentId) =>
        children.map((childId) => ({ mediaTypeTreeId: event.treeId, parentId, childId })),
      )
