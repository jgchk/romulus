...
import {
  ...
  useReleaseTypesQuery,
} from '../../services/release-types'
import { useIntRouteParam } from '../../utils/routes'

const ReleaseTypesTable: FC = () => {
  const releaseTypesQuery = useReleaseTypesQuery()

  const page = useIntRouteParam('page')
  const size = useIntRouteParam('size')

  if (releaseTypesQuery.data) {
    return (
      <HasData releaseTypes={releaseTypesQuery.data} page={page} size={size} />
    )
  }

  if (releaseTypesQuery.error) {
    return <div>Error fetching release types :(</div>
  }

  return <CenteredLoader />
}

const columnHelper = createColumnHelper<DefaultReleaseType>()

const HasData: FC<{
  releaseTypes: DefaultReleaseType[]
  page?: number
  size?: number
...
const DeleteButton: FC<{ id: number; name: string }> = ({ id, name }) => {
  const { mutate, isLoading } = useDeleteReleaseTypeMutation()
  ...
  const handleClick = useCallback(
    () =>
      mutate(
        ...
        {
          onSuccess: () => {
            toast.success(`Deleted release type '${name}'`)
          },
        }
      ),
    [id, mutate, name]
